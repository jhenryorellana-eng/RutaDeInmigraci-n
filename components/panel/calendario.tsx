...
export type CeldaDia =
  | { estado: "cita"; iso: string; nombre: string; pais: string; enEeuu: boolean }
  ...
  return (
    <main className="pt-6">
      ...
      <div className="mt-5 hidden overflow-hidden rounded-[20px] border border-white/12 md:block">
        ...
        >
          <span className="px-3 py-2.5 text-[11px] font-medium tracking-[0.14em] text-tinta-tenue">
            MT
          </span>
          {dias.map((d) => (
            <span
...
function Celda({
  ...
  if (celda.estado === "cita") {
    return (
      <span className="m-[3px] min-w-0 rounded-xl border border-acento/45 bg-acento/[0.16] px-2.5 py-2">
...
function FilaTelefono({
  ...
  if (celda.estado === "cita") {
    return (
      <div className="flex items-center gap-3.5 rounded-2xl border border-acento/45 bg-acento/[0.16] px-4 py-3.5">
